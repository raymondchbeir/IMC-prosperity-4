...
class Trader:
    ...
    USE_HG_TRADER    = False     # v12 take-style HG (kept disabled, was -EV)
    ...
    # === v36 PHASE 1: Portfolio-greek risk cap ===
    # Computes aggregate delta from all targeted positions:
    #     Δ = target_velvet + Σ (target_K * delta_BS(S, K, σ_K))
    # If |Δ| > MAX, scale all targets proportionally so the resulting Δ
    # equals MAX_PORTFOLIO_DELTA in magnitude.  This is a SAFETY NET — it
    # bounds tail risk when the peak/valley signal stacks correlated short
    # positions across VELVET + 7 vouchers (typical aggregate Δ ≈ 600-800).
    # Phase 2 (next round) will use the freed budget for delta-hedged IV arb
    # and delta-equivalent strike sizing — that's where the real alpha is.
    USE_PORTFOLIO_GREEKS    = True
    MAX_PORTFOLIO_DELTA     = 1500    # was 700 — that bit on every peak (-$180k regression).
                                      # Normal peak signal stacks ≈ 800-1200 aggregate Δ; 1500
                                      # is wide enough to leave normal trades untouched and
                                      # only bound true tail (multiple stacked signals).
    PORTFOLIO_GREEKS_LOG_EVERY_TS = 50_000   # diagnostic log cadence

    # ============================================================
    # === v37 STEP 1: GAMMA SCALPING LAYER ===
    # ============================================================
    # User's options_features.py chart showed strikes 5100/5200/5300/5400 had
    # net gamma-scalp EV of $15-22 per option (realized vol > implied vol on
    # body strikes).  Capture by:
    #   1. Hold +GAMMA_SCALP_SIZE long on each scalp strike continuously.
    #   2. Compute aggregate delta from those positions using BS Δ at
    #      current S and IV.
    #   3. Set VELVET hedge offset = -aggregate_delta to stay DELTA-NEUTRAL.
    #   4. As VELVET moves, voucher deltas change → next tick computes fresh
    #      deltas → hedge offset updates → executor rebalances VELVET.  That
    #      re-hedging is what captures the 0.5·Γ·(ΔS)² gamma P&L.
    # ARCHITECTURE: same single-executor pattern as v35 HG signal-injection.
    # We DO NOT run our own executor.  We just write desired positions into
    # st['gamma_target_pos'] and st['gamma_velvet_hedge']; the existing
    # voucher loop and VELVET exec read them as offsets.
    USE_GAMMA_SCALP         = True
    GAMMA_SCALP_STRIKES     = (5100, 5200, 5300, 5400)   # body strikes from EV chart
    GAMMA_SCALP_SIZE        = 15        # was 5 — sized up for measurable signal
    GAMMA_SCALP_IDLE_TOL    = 30        # only fire when |target_pos| <= this (no
                                        # directional signal active).  When peak/valley
                                        # fires, scalp positions flatten so they don't
                                        # interfere with the directional bet.  This
                                        # isolates the gamma alpha to quiet periods,
                                        # which is where the chart's EV was averaged from.
    GAMMA_SCALP_LOG_EVERY_TS= 20_000    # diagnostic log cadence
    ...
    HG_FOLLOW_BUYERS       = ("Mark 14",)        # follow these buys
    HG_FOLLOW_SELLERS      = ("Mark 14",)        # follow these sells
    HG_FADE_BUYERS         = ("Mark 38",)        # fade their buys (sell)
    HG_FADE_SELLERS        = ("Mark 38",)        # fade their sells (buy)
    HG_FOLLOW_SIZE         = 6                   # size per Mark 14 event
    HG_FADE_SIZE           = 4                   # size per Mark 38 event (smaller — fade weaker)
    HG_POS_LIMIT           = 200                 # Prosperity hard cap
    HG_TARGET_CAP          = 60                  # |target| cap to limit exposure
    HG_DECAY_TS            = 50_000              # decay target toward 0 if no event for this long
    HG_DECAY_FRAC          = 0.85                # multiply target by this each decay step
    ...
    def _fresh(self):
        return {
            ...
            # v36 portfolio greeks
            'greeks_log': [], 'greeks_last_log_ts': -10**9,
            # v37 gamma scalp layer — desired long-gamma positions and VELVET hedge offset
            'gamma_target_pos': {},          # str(K) -> int  (long contracts per scalp strike)
            'gamma_velvet_hedge': 0,         # int — VELVET position offset to neutralize Δ
            'gamma_agg_delta': 0.0,          # most-recent aggregate Δ from scalp layer
            'gamma_scalp_log': [],
            'gamma_scalp_last_log_ts': -10**9,
        }
    ...
    def _delta(self, S, K, sigma):
        """BS delta for a call.  Used by portfolio-greeks risk cap (v36).
        Returns the d_call/d_S value in [0, 1].  Falls back to indicator
        function when sigma or T is degenerate."""
        if sigma <= 0 or self.T_OPT <= 0:
            return 1.0 if S > K else 0.0
        sq = math.sqrt(self.T_OPT)
        d1 = (math.log(S/K) + 0.5*sigma*sigma*self.T_OPT) / (sigma*sq)
        return self._ncdf(d1)

    def _run_gamma_scalp_layer(self, st, ts, S, pts):
        """v37 STEP 1+2A: maintain long-gamma positions on body strikes
        ONLY DURING IDLE PERIODS (no directional signal active).

        Updates st['gamma_target_pos'] (per-strike contract count) and
        st['gamma_velvet_hedge'] (VELVET delta-hedge offset).  The values
        are READ by the existing executors (voucher loop and VELVET exec)
        and added to their per-tick targets — same single-executor pattern
        as v35 HG signal injection.  Does NOT emit orders or modify
        target_pos / voucher_target_pos directly.

        v37 STEP 2A: idle-period gating.  When |target_pos| > IDLE_TOL,
        a directional peak/valley signal is active and the gamma scalp
        layer flattens itself (sets all targets and hedge to 0).  This
        prevents the small gamma overlay from interfering with large
        directional bets.  The chart's EV was averaged across all ticks
        but the scalp alpha is concentrated in quiet periods — gating
        isolates that alpha from directional noise.
        """
        if not self.USE_GAMMA_SCALP:
            return

        # Idle-period gate: if peak/valley signal active, flatten and bail.
        if abs(int(st.get('target_pos', 0))) > self.GAMMA_SCALP_IDLE_TOL:
            targets = st.setdefault('gamma_target_pos', {})
            if any(targets.values()) or st.get('gamma_velvet_hedge', 0) != 0:
                for k in list(targets.keys()):
                    targets[k] = 0
                st['gamma_velvet_hedge'] = 0
                st['gamma_agg_delta'] = 0.0
                # Diagnostic at coarse cadence
                last_log = st.get('gamma_scalp_last_log_ts', -10**9)
                if ts - last_log >= self.GAMMA_SCALP_LOG_EVERY_TS:
                    st.setdefault('gamma_scalp_log', []).append({
                        'ts': ts, 'kind': 'idle_yield',
                        'velvet_target': int(st.get('target_pos', 0)),
                    })
                    st['gamma_scalp_last_log_ts'] = ts
            return

        # Build IV lookup from this tick's pts (computed earlier in run()).
        # If a strike's book is invalid, its IV is missing → skip it.
        iv_by_K = {p['K']: max(p['iv'], self.MIN_IV) for p in pts}

        # Step 1: target +SIZE long on each scalp strike where IV is available
        targets = st.setdefault('gamma_target_pos', {})
        for K in self.GAMMA_SCALP_STRIKES:
            if K in iv_by_K:
                targets[str(K)] = self.GAMMA_SCALP_SIZE
            else:
                targets[str(K)] = 0  # no IV → don't trade

        # Step 2: aggregate Δ from those positions (BS Δ at current S, σ_K)
        agg_delta = 0.0
        for K_str, t in targets.items():
            K = int(K_str)
            if t == 0 or K not in iv_by_K:
                continue
            agg_delta += t * self._delta(S, K, iv_by_K[K])

        # Step 3: store hedge offset (negative Δ → short VELVET to neutralise)
        st['gamma_velvet_hedge'] = -int(round(agg_delta))
        st['gamma_agg_delta']    = round(agg_delta, 2)

        # Step 4: diagnostic log at coarse cadence
        last_log = st.get('gamma_scalp_last_log_ts', -10**9)
        if ts - last_log >= self.GAMMA_SCALP_LOG_EVERY_TS:
            st.setdefault('gamma_scalp_log', []).append({
                'ts': ts, 'agg_delta': round(agg_delta, 2),
                'velvet_hedge': st['gamma_velvet_hedge'],
    ...
    def run(self, state: TradingState):
        ...
        # v36 PHASE 1: Portfolio-greek risk cap.
        # ============================================================
        # All targets are now finalised (signal + trim + late-cap + Mark micro
        # + IV smile arb).  Compute aggregate delta from the COMBINED targets
        # using each strike's current implied vol (from `pts` built earlier).
        # If |Δ| exceeds MAX_PORTFOLIO_DELTA, scale every target proportionally.
        # The scale-down is uniform — it preserves the *relative* sizing the
        # signal layers chose, just bounds total exposure.
        if self.USE_PORTFOLIO_GREEKS and len(pts) > 0:
            iv_by_K = {p['K']: max(p['iv'], self.MIN_IV) for p in pts}
            agg_delta = float(int(st.get('target_pos', 0)))   # VELVET = delta 1.0
            if self.USE_VOUCHER_OVERLAY:
                for K in self.VOUCHER_OVERLAY_STRIKES:
                    sigma = iv_by_K.get(K)
                    if sigma is None:
                        continue
                    v_target = int(st.get('voucher_target_pos', {}).get(str(K), 0))
                    if v_target == 0:
                        continue
                    agg_delta += v_target * self._delta(S, K, sigma)

            abs_delta = abs(agg_delta)
            if abs_delta > self.MAX_PORTFOLIO_DELTA:
                scale = self.MAX_PORTFOLIO_DELTA / abs_delta
                # Scale VELVET and voucher targets uniformly
                old_velvet = int(st.get('target_pos', 0))
                st['target_pos'] = int(round(old_velvet * scale))
                if self.USE_VOUCHER_OVERLAY:
                    for K in self.VOUCHER_OVERLAY_STRIKES:
                        old_v = int(st['voucher_target_pos'].get(str(K), 0))
                        if old_v != 0:
                            st['voucher_target_pos'][str(K)] = int(round(old_v * scale))
                # Diagnostic log (capped — only at coarse cadence to avoid bloat)
                last_log = st.get('greeks_last_log_ts', -10**9)
                if ts - last_log >= self.PORTFOLIO_GREEKS_LOG_EVERY_TS:
                    st.setdefault('greeks_log', []).append({
                        'ts': ts, 'agg_delta': round(agg_delta, 1),
                        'scale': round(scale, 3), 'cap': self.MAX_PORTFOLIO_DELTA,
                    })
                    st['greeks_last_log_ts'] = ts

        # ============================================================
        # v37 STEP 1: Run gamma scalp layer.  Updates st['gamma_target_pos']
        # (per-strike long contract count) and st['gamma_velvet_hedge']
        # (VELVET delta-hedge offset).  These are READ at execution time
        # below.  Single-executor pattern — no orders emitted from here.
        # ============================================================
        if self.USE_GAMMA_SCALP and len(pts) > 0:
            self._run_gamma_scalp_layer(st, ts, S, pts)

        # Execute VELVET to target.  v37: ADD gamma_velvet_hedge offset so the
        # gamma scalp layer's long-Δ exposure is neutralised.
        velvet_target = int(st.get('target_pos', 0)) + int(st.get('gamma_velvet_hedge', 0))
        ...
        # v37: ADD gamma_target_pos[K] to the directional voucher_target_pos[K]
        # so each scalp strike holds its long-gamma position alongside any
        # peak/valley directional bet.  Single-executor pattern.
        if self.USE_VOUCHER_OVERLAY:
            gamma_targets = st.get('gamma_target_pos', {}) if self.USE_GAMMA_SCALP else {}
            for K in self.VOUCHER_OVERLAY_STRIKES:
                v_target = int(st['voucher_target_pos'].get(str(K), 0))
                v_target += int(gamma_targets.get(str(K), 0))
    ...
    def _run_hg_trader(self, result, state, st, ts):
        """Mark 14 ↔ Mark 38 fader on HYDROGEL_PACK.
        Independent of VELVET signal logic. Empirical edge:
          Mark 14 fwd return on HG: +8.15 bps (FOLLOW)
          Mark 38 fwd return on HG: -8.01 bps (FADE)
          Spread $15.7, mid ~$10000 -> ~$64 net per round-trip.
        """
        target = int(st.get('hg_target_pos', 0))
        last_sig = int(st.get('hg_last_signal_ts', -10**9))

        # Time-decay target toward zero if no recent Mark activity
        if ts - last_sig > self.HG_DECAY_TS and abs(target) > 0:
            target = int(round(target * self.HG_DECAY_FRAC))
            st['hg_target_pos'] = target

        # Scan market_trades for Mark events on HG
        mt = getattr(state, 'market_trades', None) or {}
        hg_trades = mt.get(self.HG_PRODUCT, []) or []
        for tr in hg_trades:
            buyer  = getattr(tr, 'buyer', '') or ''
            seller = getattr(tr, 'seller', '') or ''
            # FOLLOW Mark 14: buy when they buy, sell when they sell
            if buyer in self.HG_FOLLOW_BUYERS:
                target = min(self.HG_TARGET_CAP, target + self.HG_FOLLOW_SIZE)
                st['hg_last_signal_ts'] = ts
            elif seller in self.HG_FOLLOW_SELLERS:
                target = max(-self.HG_TARGET_CAP, target - self.HG_FOLLOW_SIZE)
                st['hg_last_signal_ts'] = ts
            # FADE Mark 38: sell when they buy, buy when they sell
            elif buyer in self.HG_FADE_BUYERS:
                target = max(-self.HG_TARGET_CAP, target - self.HG_FADE_SIZE)
                st['hg_last_signal_ts'] = ts
            elif seller in self.HG_FADE_SELLERS:
                target = min(self.HG_TARGET_CAP, target + self.HG_FADE_SIZE)
                st['hg_last_signal_ts'] = ts
        st['hg_target_pos'] = target

        # Execute toward HG target (no fair-anchor protection — HG is independent)
        od = state.order_depths.get(self.HG_PRODUCT)
        if od is None: return
        bb, ba = self._bbb(od)
        if bb is None or ba is None: return
        pos = int(state.position.get(self.HG_PRODUCT, 0))
        target = int(self._clip(target, -self.HG_POS_LIMIT, self.HG_POS_LIMIT))
        need = target - pos
        if need == 0: return
        if need > 0:
            need = min(need, self.HG_POS_LIMIT - pos)
            take_qty = min(self.HG_MAX_TAKE_PER_TICK, max(1, int(abs(need) * self.HG_TAKE_FRACTION)))
            visible_ask = -int(od.sell_orders.get(ba, 0))
            if take_qty > 0 and visible_ask > 0:
                q = min(take_qty, visible_ask, need)
                self._add(result, self.HG_PRODUCT, ba, q)
                need -= q
            if need > 0:
                q = min(self.HG_MAX_PASSIVE_PER_TICK, need)
                px = bb + 1 if bb + 1 < ba else bb
                self._add(result, self.HG_PRODUCT, px, q)
        else:
            need = max(need, -self.HG_POS_LIMIT - pos)
            sell_need = -need
            take_qty = min(self.HG_MAX_TAKE_PER_TICK, max(1, int(sell_need * self.HG_TAKE_FRACTION)))
            visible_bid = int(od.buy_orders.get(bb, 0))
            if take_qty > 0 and visible_bid > 0:
                q = min(take_qty, visible_bid, sell_need)
                self._add(result, self.HG_PRODUCT, bb, -q)
                sell_need -= q
            if sell_need > 0:
                q = min(self.HG_MAX_PASSIVE_PER_TICK, sell_need)
