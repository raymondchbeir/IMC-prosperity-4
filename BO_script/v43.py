"""v43 — v42 + HG anti-chop fix A: gated L1 fair-touch unload.

CHANGE vs v42 (single isolated change in _run_hg_mm L1):
  L1 fair-touch unload was firing every time bid >= fair, even on chop.
  With HG_CLIP_RANGE=5 cum-mid fair, "bid >= fair" is true ~half the time
  HG ranges around its mean. We were dumping up to 100 units the moment
  price kissed fair, then L2 rebuilt on the next dev>=8 print. Round-trip
  bleed on horizontal movement.

  v43 adds two gates to L1:
    (1) Profit buffer: only unload when bid > fair + HG_FAIR_TOUCH_BUFFER
        (default 3). A bare touch of fair is no longer enough — the print
        must be meaningfully through fair.
    (2) Min hold time: only unload positions opened at least
        HG_FAIR_TOUCH_MIN_HOLD_TS ticks ago (default 3000). Stops the
        instant-flip where we enter at dev=-8, fair touches a few hundred
        ticks later, we dump, and re-enter at dev=+8 immediately after.

  L2 logic, MM logic, and signal logic are unchanged.

EXPECTED:
  - HG round-trip count down ~30-50% on flat days
  - Per-trade $ up (remaining unloads are real edge captures, not noise)
  - Slightly bigger drawdowns on real reversals (cost of holding through
    fair touches), should be more than offset by reduced chop bleed

TOGGLES:
  - HG_FAIR_TOUCH_BUFFER       = 0.0 reverts L1 unload edge to v42
  - HG_FAIR_TOUCH_MIN_HOLD_TS  = 0   reverts hold gate to v42
...
class Trader:
    ...
    def _run_hg_mm(self, result, state, st):
        ...
        # Reset when flat; set on first non-flat tick.
        ...
        if pos == 0:
            ...
            # direction flipped — restart hold clock
            st['hg_pos_open_ts'] = ts
        st['hg_prev_pos'] = pos
