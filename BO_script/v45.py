"""v45 — v44 + HG anti-chop fixes C and D: flip cooldown + min resize.

CHANGES vs v42 (all in _run_hg_mm), stacked:

(A) v43: gate L1 fair-touch unload with profit buffer (HG_FAIR_TOUCH_BUFFER=3)
    AND minimum hold time (HG_FAIR_TOUCH_MIN_HOLD_TS=3000).

(B) v44: hysteresis between entry and exit:
      ENTRY_BAND   8.0 -> 12.0
      OFFLOAD_BAND 10.0 ->  6.0
      FLAT_BAND     5.0 ->  3.0

(C) NEW v45: direction-flip cooldown.
    After the L2 target sign flips (long -> short or vice-versa), force the
    target back toward zero for HG_FLIP_COOLDOWN_TS (5000) ticks. The
    fair-clipped cum-mid CAN sometimes flip sign during chop right after a
    fair-touch unload — without this gate L2 immediately rebuilds the
    opposite side and we round-trip again.

(D) NEW v45: min resize threshold.
    Skip L2 execution if the desired delta to current position is below
    HG_MIN_RESIZE (12 units). Tiny rebalances cross the spread for no
    real edge and add to the chop bleed.

EXPECTED:
  - Lowest HG round-trip count of the v43/v44/v45 ladder
  - Smaller average per-tick exposure during chop
  - Slightly slower-to-react on a clean direction reversal (cost of the
    flip cooldown), but in chop the cooldown is exactly what we want.

TOGGLES:
  - HG_FLIP_COOLDOWN_TS = 0  reverts (C)
  - HG_MIN_RESIZE = 1        reverts (D)
  - All v43/v44 toggles still apply
...
class Trader:
    ...
    def _run_hg_mm(self, result, state, st):
        ...
        HG_FAIR_TOUCH_BUFFER      = 3.0
        HG_FAIR_TOUCH_MIN_HOLD_TS = 3_000
        # v45 anti-chop gates on L2 sign-flip and tiny resizes
        HG_FLIP_COOLDOWN_TS = 5_000
        HG_MIN_RESIZE       = 12
        ...
        # ====================================================================
        # v45 (C): direction-flip cooldown.
        # If L2 target sign just flipped vs the last non-zero target we
        # actually pursued, force target back toward zero for HG_FLIP_COOLDOWN_TS
        # ticks.  This avoids the round-trip pattern where L1 unloads, fair
        # drifts past mid, and L2 immediately rebuilds the opposite side.
        # ====================================================================
        if day_ts < END_FLAT_TS:
            new_sign = (1 if target > 0 else (-1 if target < 0 else 0))
            last_sign = int(st.get('hg_last_target_sign', 0))
            last_flip_ts = int(st.get('hg_last_flip_ts', -10**9))
            if new_sign != 0 and last_sign != 0 and new_sign != last_sign:
                if (ts - last_flip_ts) < HG_FLIP_COOLDOWN_TS:
                    target = 0
                else:
                    st['hg_last_flip_ts'] = ts
                    st['hg_last_target_sign'] = new_sign
            elif new_sign != 0:
                st['hg_last_target_sign'] = new_sign
        ...
        # v45 (D): skip tiny rebalances that just bleed spread.
        # Don't skip when reducing (we still want to flatten near close)
        # or in the very_late flatten window.
        if (not very_late) and (not reducing) and 0 < abs(need) < HG_MIN_RESIZE:
            need = 0
            target = sim_pos
