...
class Trader:
    ...
    # === v17 patch: extended to all idle strikes.  4500 mirrors the VEV_4000
    # treatment (deep ITM, wide spread).  6000/6500 are deep OTM lottery tickets
    # with a tight 0/1 book, so they need join-touch quoting and the
    # below-fair-sell gate disabled.  Per-strike config overrides defaults.
    USE_VOUCHER_MM            = True
    VOUCHER_MM_STRIKES        = (4500, 5400, 5500, 6000, 6500)
    VOUCHER_MM_QUOTE_SIZE     = 4    # default; overridden per-strike below
    VOUCHER_MM_MIN_SPREAD     = 1
    VOUCHER_MM_IMPROVE_TICKS  = 1
    VOUCHER_MM_POS_SOFT       = 100
    VOUCHER_MM_POS_HARD       = 200
    VOUCHER_MM_CONVERGED_TOL  = 5

    # Per-strike MM overrides.  Strikes absent here use the defaults above.
    #   quote_size            : base size per side
    #   min_spread            : skip MM when spread < this
    #   improve_ticks         : 0 = join touch, 1 = bid+1/ask-1
    #   allow_below_fair_sell : if True, skip the NEVER_SELL_BELOW_FAIR gate.
    #                           Use for deep-ITM (4500) and deep-OTM (6000/6500)
    #                           where the gate's reasoning ("voucher is cheap
    #                           when VELVET below fair") doesn't apply.
    #   join_touch            : if True, quote at touch instead of improving
    #                           (needed when ask-bid == 1 — improving by 1
    #                           would cross and bail).
    VOUCHER_MM_PER_STRIKE = {
        4500: {'quote_size': 5, 'min_spread': 4, 'improve_ticks': 1,
               'allow_below_fair_sell': True,  'join_touch': False},
        5400: {'quote_size': 4, 'min_spread': 1, 'improve_ticks': 1,
               'allow_below_fair_sell': False, 'join_touch': False},
        5500: {'quote_size': 4, 'min_spread': 1, 'improve_ticks': 1,
               'allow_below_fair_sell': False, 'join_touch': False},
        6000: {'quote_size': 3, 'min_spread': 1, 'improve_ticks': 0,
               'allow_below_fair_sell': True,  'join_touch': True},
        6500: {'quote_size': 3, 'min_spread': 1, 'improve_ticks': 0,
               'allow_below_fair_sell': True,  'join_touch': True},
    }
    ...
    VOUCHER_CORE_STRIKES      = (5000, 5100, 5200)
    VOUCHER_PEAK_EXTRA_STRIKES= (4000, 4500, 5300, 5400)
    ...
    def _voucher_passive_mm(self, result, state, st, S, in_chop, K):
        """Two-sided MM on a voucher strike, parameterised per strike.

        v17 patch: reads VOUCHER_MM_PER_STRIKE for quote_size/min_spread/
        improve_ticks/allow_below_fair_sell/join_touch.  Strikes not in that
        dict fall back to the legacy class-level defaults.

        Quotes when:
          - main voucher target is converged (|target - pos| <= tol)
          - spread >= per-strike min_spread
          - not in chop
        Inventory-skewed.  NEVER_SELL_BELOW_FAIR honored unless overridden.
        """
        if in_chop: return
        cfg = self.VOUCHER_MM_PER_STRIKE.get(K, {
            'quote_size':            self.VOUCHER_MM_QUOTE_SIZE,
            'min_spread':            self.VOUCHER_MM_MIN_SPREAD,
            'improve_ticks':         self.VOUCHER_MM_IMPROVE_TICKS,
            'allow_below_fair_sell': False,
            'join_touch':            False,
        })
        ...
        # NEVER_SELL_BELOW_FAIR gate — skipped on strikes flagged as ITM/OTM
        # extremes where the "voucher is cheap when VELVET < FAIR" reasoning
        # doesn't hold (deep ITM tracks intrinsic; deep OTM is a $0.50 lottery).
        if not cfg['allow_below_fair_sell']:
            if self.NEVER_SELL_BELOW_FAIR and S < self.FAIR + self.MIN_SELL_EDGE:
                ask_size = 0
            if S > self.FAIR + 30:
                bid_size = 0

        # Quote prices — join touch when spread is too tight to improve
        if cfg['join_touch']:
            bid_px = u_bb
            ask_px = u_ba
        else:
            improve = cfg['improve_ticks']
            bid_px = u_bb + improve
            ask_px = u_ba - improve
    ...
    def run(self, state: TradingState):
        ...
        # v13: passive MM on HG
        if self.USE_HG_MM:
            self._run_hg_mm(result, state)
    ...
    def _run_hg_mm(self, result, state):
        """Directional MM on HYDROGEL_PACK (v14).
        Posts bid+1 / ask-1 to capture spread, AND skews inventory toward
        a target = -K × (mid - $9994). Below fair → want long. Above → want short.
        """
        ...
        if bb is None or ba is None: return
        spread = ba - bb
        if spread < self.HG_MM_MIN_SPREAD: return

        pos = int(state.position.get('HYDROGEL_PACK', 0))
        cap = 200
        mid = 0.5 * (bb + ba)

        # Compute target inventory from deviation
        deviation = mid - self.HG_FAIR
        target_pos = int(round(-self.HG_TILT_K * deviation))
        target_pos = max(-self.HG_TILT_CAP, min(self.HG_TILT_CAP, target_pos))
        error = target_pos - pos    # positive => need more long

        # Quote prices — improve by 1 tick
        bid_px = bb + self.HG_MM_IMPROVE_TICKS
        ask_px = ba - self.HG_MM_IMPROVE_TICKS
        if bid_px >= ask_px: return

        # Skewed sizing: bid gets boosted when we want more long,
        # ask gets boosted when we want less long (more short).
        bid_size = self.HG_MM_QUOTE_SIZE
        ask_size = self.HG_MM_QUOTE_SIZE
        if error > 0:
            # Need more long
            bid_size += min(self.HG_TILT_BID_BOOST, error)
            ask_size = max(0, self.HG_MM_QUOTE_SIZE - min(self.HG_TILT_ASK_BOOST, error // 2))
        elif error < 0:
            # Need less long (more short)
            ask_size += min(self.HG_TILT_ASK_BOOST, -error)
            bid_size = max(0, self.HG_MM_QUOTE_SIZE - min(self.HG_TILT_BID_BOOST, (-error) // 2))

        # Hard cap — full risk control
        if pos >= self.HG_MM_POS_HARD: bid_size = 0
        if pos <= -self.HG_MM_POS_HARD: ask_size = 0
        ...
        if bid_size > 0:
            self._add(result, 'HYDROGEL_PACK', bid_px, bid_size)
        if ask_size > 0:
            self._add(result, 'HYDROGEL_PACK', ask_px, -ask_size)

        # === v15: aggressive take at extreme deviations ===
        # When |dev| ≥ HG_TAKE_THRESHOLD, cross the spread to take liquidity
        # in the reversion direction. This guarantees position fills when
        # passive MM doesn't catch a Mark crossing.
        abs_dev = abs(deviation)
        if abs_dev >= self.HG_TAKE_THRESHOLD:
            if deviation > 0:
                # HG above fair → SELL aggressively (hit the displayed bid)
                if pos > -self.HG_TAKE_CAP:
                    visible_bid = int(od.buy_orders.get(bb, 0))
                    if visible_bid > 0:
                        room = self.HG_TAKE_CAP + pos  # how much we can still sell
                        q = min(self.HG_TAKE_SIZE, visible_bid, max(0, room))
                        if q > 0:
                            self._add(result, 'HYDROGEL_PACK', bb, -q)
            else:
                # HG below fair → BUY aggressively (hit the displayed ask)
                if pos < self.HG_TAKE_CAP:
                    visible_ask = -int(od.sell_orders.get(ba, 0))
                    if visible_ask > 0:
                        room = self.HG_TAKE_CAP - pos
                        q = min(self.HG_TAKE_SIZE, visible_ask, max(0, room))
                        if q > 0:
                            self._add(result, 'HYDROGEL_PACK', ba, q)
